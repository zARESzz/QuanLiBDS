...
namespace QuanLy
{
    public partial class frmKHTN : DevExpress.XtraEditors.XtraForm
    {
        public frmKHTN()
        {
            InitializeComponent();
        ...
        void loadData()
        {
            gcKHTN.DataSource = _tn.getList();
            gvKHTN.OptionsBehavior.Editable = false;
            _listtn = _tn.getList();
        }
        ...
        {
            id = int.Parse(gvKHTN.GetFocusedRowCellValue("MaTN").ToString());
            txtHoTen.Text = gvKHTN.GetFocusedRowCellValue("HoTenTN").ToString();
            txtDiaChi.Text = gvKHTN.GetFocusedRowCellValue("DiaChi").ToString();
            txtEmail.Text = gvKHTN.GetFocusedRowCellValue("Email").ToString();
            txtSDT.Text = gvKHTN.GetFocusedRowCellValue("SDT").ToString();
        }

        private void btnIn_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            rptTiemNang rpt = new rptTiemNang(_listtn);
            rpt.ShowPreview();
        }
        ...
        private void cboSheet_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (dt != null)
            {
                List<KHTN> khachHangs = new List<KHTN>();
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    KHTN kHACHHANG = new KHTN();
                    kHACHHANG.MaTN = int.Parse(dt.Rows[i]["MaTN"].ToString());
                    kHACHHANG.HoTenTN = dt.Rows[i]["HoTenTN"].ToString();
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DapperPlusManager.Entity<KHTN>().Table("KHTN");
                List<KHTN> khachHangs = gcKHTN.DataSource as List<KHTN>;
                if (khachHangs != null)
                {
