...
CREATE PROC P_MaPhieu
AS
BEGIN
DECLARE @MA_NEXT VARCHAR(5)
DECLARE @MAX INT
SELECT @MAX = COUNT(MaPhieu ) FROM PHIEUDENBU WHERE MaPhieu  LIKE 'PDB'
SET @MA_NEXT = 'PDB' + RIGHT('0'+CAST(@MAX AS VARCHAR(3)),3)
WHILE(EXISTS (SELECT MaPhieu  FROM PHIEUDENBU WHERE MaPhieu =@MA_NEXT))
BEGIN
	SET @MAX = @MAX+1
	SET @MA_NEXT = 'PDB' + RIGHT('0'+CAST(@MAX AS VARCHAR(3)),3)
END
