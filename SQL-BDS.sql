...
(
	...
	HoTenTN	NVARCHAR(36) NOT NULL,
	SDT		VARCHAR(15) NOT NULL CONSTRAINT UNI_SDTTN UNIQUE,
	DiaChi  NVARCHAR(50),
	Email	VARCHAR(35) NOT NULL CONSTRAINT UNI_EMAILTN UNIQUE
)
GO
...
(
	...
	MaTN	INT CONSTRAINT FK_PH_TN FOREIGN KEY (MaTN) REFERENCES KHTN(MaTN),
	MaTK	VARCHAR(5) CONSTRAINT FK_PH_TK FOREIGN KEY (MaTK) REFERENCES NHANVIEN(MaTK),
...
DECLARE @MAX INT
SELECT @MAX = COUNT(MaPH) FROM PHIEUHEN WHERE MaPH LIKE 'CV'
SET @MA_NEXT = 'PH' + RIGHT('0'+CAST(@MAX AS VARCHAR(3)),3)
WHILE(EXISTS (SELECT MaPH FROM PHIEUHEN WHERE MaPH=@MA_NEXT))
BEGIN
	SET @MAX = @MAX+1
	SET @MA_NEXT = 'PH' + RIGHT('0'+CAST(@MAX AS VARCHAR(3)),3)
END
...
CREATE FUNCTION F_THONGKETHANG
(
@NGAYD DATETIME,
@NGAYC DATETIME
)
RETURNS TABLE AS RETURN
SELECT L.TenLoai ,SUM(H.PHI) AS THANHTIEN FROM HOPDONG H, BATDONGSAN B, LOAIBDS L
WHERE H.MaBDS = B.MaBDS AND B.MaLoai = L.MaLoai AND H.NgayLap>=CONVERT(DATETIME,@NGAYD,103) AND H.NgayLap<CONVERT(DATETIME,@NGAYC,103)
GROUP BY L.TenLoai

SELECT* FROM F_THONGKETHANG ('2022-10-01','2022-10-14')

CREATE FUNCTION F_THONGKEDOANHTHU
(
@NGAYD DATETIME,
@NGAYC DATETIME
)
